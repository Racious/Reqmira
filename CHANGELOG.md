...
- **Body 編輯器升級為 CodeMirror**：JSON 語法高亮、括號配對、行號、自動縮排（節制引入，維持輕量）
- 頂列顯示版本號（取自 Tauri 設定，與發佈一致）

### 介面調整

- History 列瘦身：移除列內「載入」鈕，名稱不再被擠壓
- 「載入請求」移至回應區頂部（設為基準旁），改為紫色強調鈕，按下把該筆當時的請求載入編輯器
